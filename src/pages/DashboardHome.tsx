    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl p-6 bg-primary shadow-xl">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">{typeof user?.name === 'object' && user?.name !== null ? ((user?.name as any)?.ar || (user?.name as any)?.en || '') : (user?.name || '')} 👋</h1>
                ...
                {branches.map(branch => (
                  <tr key={branch.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-slate-800 dark:text-white">{typeof branch.name === 'object' && branch.name !== null ? ((branch.name as any)?.ar || (branch.name as any)?.en || '') : (branch.name || '')}</p>
