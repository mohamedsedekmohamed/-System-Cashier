...
import { adminApi, ADMINS_KEY, ADMINS_SELECT_OPTIONS_KEY } from '../../services/adminService';
import type { AdminFormData } from '../../types';
import { MdArrowForward, MdSave, MdSupervisorAccount } from 'react-icons/md';
...
const AdminAdd: React.FC = () => {
  ...
  const { data: optionsData, isLoading: isLoadingOptions } = useQuery({
    queryKey: [ADMINS_SELECT_OPTIONS_KEY],
    queryFn: adminApi.getSelectOptions,
  });

  const roles = optionsData?.data?.roles ?? [];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.password || !formData.role_id) return; // Basic validation
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <MdSupervisorAccount size={24} className="text-primary" />
              <h1 className="text-xl font-bold text-slate-800 dark:text-white">إضافة مدير جديد</h1>
            </div>
            <p className="text-sm text-slate-500 mt-1">أدخل بيانات المدير الجديد وأسند له دوراً</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <select name="role_id" required
                ...
                {roles.map(role => (
                  <option key={role.id} value={role.id}>{typeof role.name === 'object' && role.name !== null ? ((role.name as any)?.ar || (role.name as any)?.en || '') : (role.name || '')}</option>
                ))}
              </select>
            </div>
          </div>
        ...
        {mutation.isError && (
          <div className="mt-6 p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm">
            حدث خطأ أثناء إضافة المدير. يرجى المحاولة مرة أخرى.
          </div>
        )}
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700 flex items-center justify-end gap-3">
          ...
          <button type="submit" disabled={mutation.isPending || !formData.name || !formData.password || !formData.role_id}
