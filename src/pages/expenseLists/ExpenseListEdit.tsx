...
const ExpenseListEdit: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: [EXPENSE_LISTS_KEY, id],
    queryFn: () => expenseListApi.get(id as string),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (expense) {
      setFormData({
        ...
        description: {
          ar: expense.description.ar || '',
          en: expense.description.en || '',
        }
      });
    }
  }, [expense]);
  ...
  const mutation = useMutation({
    mutationFn: (payload: ExpenseListPayload) => expenseListApi.update(id as string, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [EXPENSE_LISTS_KEY] });
      navigate('/dashboard/expense-lists');
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Ensure description null if empty to match schema
    const payload = {
      name: formData.name,
      description: {
  ...
  // ── Render States ──────────────────────
  if (isLoading) return <LoadingSpinner text="جاري تحميل بيانات القائمة..." />;
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link to="/dashboard/expense-lists"
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-slate-800 dark:text-white">تعديل قائمة المصروفات</h1>
            </div>
            <p className="text-sm text-slate-500 mt-1">
              تعديل بيانات {expense?.name.ar}
            </p>
          </div>
        </div>
      </div>
