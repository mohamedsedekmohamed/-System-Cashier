...
const PaymentMethodEdit: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => paymentMethodApi.get(id as string),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (method) {
      setFormData({
        ...
        description: {
          ar: method.description?.ar || '',
          en: method.description?.en || '',
        },
        icon: method.icon || '',
  ...
  const mutation = useMutation({
    mutationFn: (payload: PaymentMethodFormData) => paymentMethodApi.update(id as string, payload),
    onSuccess: () => {
  ...
  // ── Render States ──────────────────────
  if (isLoading) return <LoadingSpinner text="جاري تحميل بيانات طريقة الدفع..." />;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-slate-800 dark:text-white">تعديل طريقة الدفع</h1>
            </div>
            <p className="text-sm text-slate-500 mt-1">
              تعديل بيانات {method?.name?.ar}
            </p>
          </div>
        </div>
      </div>
