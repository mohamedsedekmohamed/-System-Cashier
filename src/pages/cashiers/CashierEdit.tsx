...
const CashierEdit: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: [CASHIERS_KEY, id],
    queryFn: () => cashierApi.get(id as string),
    enabled: !!id,
  });

  const cashier = data?.data;
  const branches = data?.select_options?.branches ?? [];
  const cashierMen = data?.select_options?.cashier_men ?? [];
  ...
  const mutation = useMutation({
    mutationFn: (payload: CashierFormData) => cashierApi.update(id as string, payload),
    onSuccess: () => {
  ...
  // ── Render States ──────────────────────
  if (isLoading) return <LoadingSpinner text="جاري تحميل بيانات الكاشير..." />;
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6">
        ...
        {mutation.isError && (
          <div className="mt-6 p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm">
            حدث خطأ أثناء تحديث بيانات الكاشير. يرجى المحاولة مرة أخرى.
          </div>
        )}
