...
const KitchenList: React.FC = () => {
  ...
  const [viewTarget, setViewTarget] = useState<Kitchen | null>(null);
  const perPage = 15;

  // ── Fetch (paginated) ──────────────────
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: [KITCHENS_KEY, page, perPage],
    queryFn: () => kitchenApi.list(page, perPage),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => kitchenApi.delete(id),
    onSuccess: () => {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: (kitchen: Kitchen) => {
      ...
      return kitchenApi.update(kitchen.id, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [KITCHENS_KEY] });
    },
  ...
  const filtered = [...(search
    ? kitchens.filter(k =>
        k.name?.ar?.toLowerCase().includes(search.toLowerCase()) ||
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          ...
          <Link to={`/dashboard/kitchens/edit/${row.id}`}
  ...
  return (
    <div className="space-y-5 flex flex-col h-full">
      <CrudHeader
        title="إدارة المطابخ"
        icon={<MdSoupKitchen size={28} />}
        totalCount={meta?.total}
        search={search}
        onSearchChange={setSearch}
        searchPlaceholder="ابحث باسم المطبخ، المعرف، أو الفرع..."
        onRefresh={() => refetch()}
        addLink="/dashboard/kitchens/add"
        addText="إضافة مطبخ"
      />
      ...
      <div className="flex-1">
        <DataTable
          ...
          emptyIcon={<MdSoupKitchen size={48} />}
        />
      </div>
      ...
      <DeleteModal
        ...
        descriptionPrefix="هل أنت متأكد من حذف المطبخ"
      />
