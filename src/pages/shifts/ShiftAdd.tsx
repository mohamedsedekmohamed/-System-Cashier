...
const ShiftAdd: React.FC = () => {
  ...
  const { data: optionsData, isLoading: isLoadingOptions } = useQuery({
    queryKey: ['shifts-select-options'],
    queryFn: () => shiftApi.getSelectOptions(),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: ShiftFormData) => shiftApi.create(data),
    onSuccess: () => {
    ...
    onError: (err: unknown) => {
      ...
      setApiError(msg || 'حدث خطأ أثناء إضافة الوردية.');
    },
  });
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-800 dark:text-white">إضافة وردية جديدة</h1>
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">أدخل بيانات الوردية الجديدة</p>
        </div>
      </div>
      ...
      {isSuccess && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400">
          <MdCheck size={20} />
          <span className="font-semibold text-sm">تم إضافة الوردية بنجاح! جاري التحويل...</span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 flex flex-col sm:flex-row items-center gap-3 sm:justify-end">
            ...
            <button id="submit-add-shift" type="submit" disabled={isPending || isSuccess}
