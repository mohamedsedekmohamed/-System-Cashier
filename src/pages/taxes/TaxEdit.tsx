...
const TaxEdit: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: [TAXES_KEY, id],
    queryFn: () => taxApi.get(id as string),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (tax) {
      setFormData({
        name: { ar: tax.name?.ar || '', en: tax.name?.en || '' },
  ...
  const mutation = useMutation({
    mutationFn: (payload: TaxFormData) => taxApi.update(id as string, payload),
    onSuccess: () => {
  ...
  if (isLoading) return <LoadingSpinner text="جاري تحميل بيانات الضريبة..." />;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-slate-800 dark:text-white">تعديل بيانات الضريبة</h1>
            </div>
            <p className="text-sm text-slate-500 mt-1">تعديل {tax?.name?.ar}</p>
          </div>
        </div>
      </div>
